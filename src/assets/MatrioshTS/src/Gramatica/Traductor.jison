...
IF : pr_if parizq E parder BLOQUE ELSE	{ $$ = new If($3,$5,$6,@1.first_line,@1.first_column); };
...
F : L_PARAMETROS parder dospuntos TIPO BLOQUE  { 
	...
	| parder dospuntos TIPO BLOQUE { 
		...
		$$ = new Funcion(parent,[],$5,@1.first_line,@1.first_column);  
	} ;
...
E : ARITMETICA      	{ $$ = $1; } 
	...
	| entero        	{ $$ = new Literal(Number(yytext),Type.NUMBER,@1.first_line,@1.first_column); } 
	| decimal	    	{ $$ = new Literal(Number(yytext),Type.NUMBER,@1.first_line,@1.first_column); }
	| char        		{ $$ = new Literal(String(yytext),Type.STRING,@1.first_line,@1.first_column); } 
	| cadena	    	{ $$ = new Literal(String(yytext),Type.STRING,@1.first_line,@1.first_column); }
	| pr_true        	{ $$ = new Literal(Boolean(true),Type.BOOLEAN,@1.first_line,@1.first_column); } 
	| pr_false	    	{ $$ = new Literal(Boolean(false),Type.BOOLEAN,@1.first_line,@1.first_column); }
	;
